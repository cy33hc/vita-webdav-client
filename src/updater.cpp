...
namespace Updater {
    ...
    int UpdateFtpClient()
    {
        ...
        current_version = FS::Load(WEBDAV_CLIENT_VERSION_PATH);
        update_version = FS::Load(WEBDAV_CLIENT_VERSION_UPDATE_PATH);
        ...
        if (strcmp(cur_ver, upd_ver) != 0)
        {
            ...
            ExtractFile(WEBDAV_CLIENT_VPK_UPDATE_PATH, APP_PATH "/", nullptr);

            FS::Save(WEBDAV_CLIENT_VERSION_PATH, upd_ver, 4);
            FS::Rm(WEBDAV_CLIENT_VPK_UPDATE_PATH);
            FS::Rm(WEBDAV_CLIENT_VERSION_UPDATE_PATH);
